...
export default async function decorate(block) {
  ...
  const storeDetails = {
    environmentId: await getConfigValue(
      "commerce.headers.cs.Magento-Environment-Id"
    ),
    ...
    websiteCode: await getConfigValue(
      "commerce.headers.cs.Magento-Website-Code"
    ),
    storeCode: await getConfigValue("commerce.headers.cs.Magento-Store-Code"),
    storeViewCode: await getConfigValue(
      "commerce.headers.cs.Magento-Store-View-Code"
    ),
    config: {
      pageSize: 8,
      perPageConfig: {
    ...
    context: {
      customerGroup: await getConfigValue(
        "commerce.headers.cs.Magento-Customer-Group"
      ),
    },
    route: ({ sku, urlKey }) => {
